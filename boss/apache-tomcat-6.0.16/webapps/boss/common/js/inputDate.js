...
var fontsize = 2;
var gNow = new Date();
var gNowDay = gNow.getDay();
var gNowMonth = gNow.getMonth();
var gNowYear  = gNow.getYear();
...
//    var datePat = /^(\d{4})(\d{2})(\d{2})$/; //YYYYMMDD
    var datePat = /^(\d{4})(\/|-)(\d{2})\2(\d{2})$/; //YYYY-MM-DD
...
//        alert(sDate + "  不是一个有效的日期格式.")
    ...
    day = matchArray[4];
	gNowDay = matchArray[4];
	gNowMonth = matchArray[3];
	gNowYear  = matchArray[1];

    if (year < 1900 || year > 2100) { // check month range
//        alert("年份应该在 1900 到 2100 之间.");
        return false;
    }
...
function show_calendar() {
	...
	*/
	p_item = arguments[0];
//	alert(eval(p_item+".value"));
	cDate = eval(p_item+".value");
	if(isValidDate(cDate)){
		gNow.setYear(gNowYear);
		gNow.setMonth(gNowMonth-1);
		gNow.setDate(gNowDay);
	}
