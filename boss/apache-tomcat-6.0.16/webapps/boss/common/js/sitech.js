...
whichIM="local"
if (false==(MsngrIsStateOnline(MsngrObj.GetLocalUserStatus()) && MIU()))
{
if (top!=self)
MsngrCreateWebObj()
}
...
DC.onmouseover=OMO;
DC.onmouseout=OMOU;
DC.innerHTML='<img src="http://64.4.55.109/i.p.instantreply.gif" border=0 align="absmiddle" hspace="1"> Instant Reply';
return;		
}		
}
...
if(c){
c.colSpan=5;
c.innerHTML = "Click on the&nbsp;<img src='http://64.4.55.109/i.p.im_on.gif'>&nbsp;icon to send an instant message.";
}
}
}
function f5()
...
G("http://by16fd.bay16.hotmail.msn.com/cgi-bin/HoTMaiL?DoEmpty=1");
}
}
...
EP="&"+EP;
var UR="http://by16fd.bay16.hotmail.msn.com/cgi-bin/dasp/error_modalshell.asp?Err="+Err+"&IsConf="+bC+EP;
try 
{
...
while (sE.tagName!="TR")
{sE=sE.parentElement}
OW(sE.id.replace(/-/g,''),"297","201",sEC.top+screenTop+2-document.body.scrollTop,sEC.left+window.screenLeft+19-document.body.scrollLeft,"no","no","no","no","no","http://by16fd.bay16.hotmail.msn.com/cgi-bin/doaddresses?_HMaction=ShowSpace&contactId="+sE.id)
}


function checkMultipleSelect( elementName , minSelect , maxSelect ){
//alert(elementName ) ;
	elements = document.all( elementName ) ;
	if( !elements ){
		if( minSelect <= 0 ){
			return true ;
		}else{
	...
	if( elements.tagName ){
		...
		if( num >= minSelect && num <= maxSelect ){
			return true ;
		}else{
			return false ;
		}
	}
//alert(elements.tagName ) ;
...
function setCurrentElement( elementName , value ){
	elements = document.all( elementName ) ;
	if( !elements ){
		return false ;
	}
...
function getSelectedCheckId( elementName ){
...
function getSelectedCheckIds( elementName ){
	elements = document.getElementsByName( elementName ) ;
	if( !elements ){
		return  ;
	}
