	   ...
	   <%
	    String func="";
		  for (int i=0;i<next_list.size();i++){
		      ...
		      if (fsm_next.getAlias().equals("personneltree")){
		           func=fsm_next.getFunc_code();
		      }
    }%>
    <frame src="list.do?listtype=personnel_service_p&func_code=<%=func%>" name="inleft" scrolling="auto" />
    <frame src="" name="inright" scrolling="auto" />
       
    <!--
    <frame src="list.do?listtype=personnel_service_s&id=" name="inright" scrolling="no" />
    -->
   </frameset>
</frameset>
</html>
