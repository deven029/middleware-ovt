...
<body>
	<table width='100%' cellspacing='0' cellpadding='0' border='0'>
			<tr>
				<td nowrap height="30">
					当前位置:
					<span class='DD'>消息管理-->OSD管理（缺省显示最新十条OSD）</span>
				</td>
			</tr>
		</table>
		</br>
		...
		<div id="condition">
			<table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
				<form name="commonQueryForm" method="post" action="<%=path %>/bossCoreMessage/osd.do?method=query">
					<tr>
						...
						<td align="center" width="16%" height="23">OSD类型: </td>
						<td align="left" width="34%" height="23" colspan="1">
							<select name="osdType" id="osdType" size="1" >
								<option value="" <c:if test="${osdType==null}"> selected </c:if> >选择类型</option>
								<option value="0" <c:if test="${osdType==0}"> selected </c:if> >单发</option><option value="1" <c:if test="${osdType==1}"> selected </c:if> >群发</option>
							</select>
						</td>
					</tr>
		...
		<table id='scrollTable'  width="99%" border="0" cellspacing="1" cellpadding="0" align="center" bgcolor="#999999">
			...
			<thead>
				<tr bgcolor="#CCCCCC">
					...
					<td nowrap align="center" height="30" width="20%">发布状态</td>
					<td nowrap align="center" height="30" width="20%">OSD类型</td>
					<td nowrap align="center" height="30" width="20%">OSD内容</td>
					<td nowrap align="center" height="30" width="20%">智能卡号/片区名称</td>
					<td nowrap align="center" height="30" width="20%">发布时间</td>
				</tr>
			</thead>
			
			<tbody>
				<logic:notEmpty name = "allOsdResults" >
					<logic:iterate id = "osd" name = "allOsdResults" scope="session"  indexId="line_no">
						<tr bgcolor="#FFFFFF" onMouseOver="openme(this);" onMouseOut="closeme(this);" id="tr${line_no}">
							<td  align="center" height="30" width="0%"><input type="checkbox" id="id" name="id" value="<bean:write name = "osd" property = "id" />"></td>
							<td  align="center" height="30" width="20%" id="tr${line_no}-1" >
								<c:if test="${osd.publicDate==null}"><font color="red">未发布</font></c:if>
								<c:if test="${osd.publicDate!=null}"><font color="green">已发布</font></c:if>
							</td>
							<td  align="center" height="30" width="20%"  >
								<c:if test="${osd.osdType==0}">单发</c:if>
								<c:if test="${osd.osdType==1}">群发</c:if>
							</td>
							<td  align="center" height="30" width="20%"><bean:write name = "osd" property = "text" /></td>
							<td  align="center" height="30" width="20%">
								<c:if test="${osd.osdType==0}">
									<bean:write name = "osd" property = "equNo" />
									<input type="hidden" name="conditionNames" id="conditionNames" value="${osd.equNo}" />
								</c:if>
								<c:if test="${osd.osdType==1}">
									<bean:write name = "osd" property = "areaName" />
									<input type="hidden" name="conditionNames" id="conditionNames" value="${osd.areaName}" />
								</c:if>
							</td>
							<td  align="center" height="30" width="20%" id="tr${line_no}-2"><bean:write name = "osd" property = "publicDate" format="yyyy-MM-dd HH:mm:ss" /></td>
						</tr>
					</logic:iterate>
				</logic:notEmpty>
			</tbody>
	...
	<script language="JavaScript">
		...
		function addView_click(){
			queryResultForm.action='<%=path%>/bossCoreMessage/osd.do?method=add';
		...
		function modifyView_click(){
			...
			queryResultForm.action='<%=path%>/bossCoreMessage/osd.do?method=modify&id=' + v;
			queryResultForm.submit();
		}
			...
			for(var i=0; i<c.length; i++){ 
	     		if(c[i].checked == true) {
	     			 if(conditionNames[i].value == ''){
						alert("所选OSD中存在【智能卡号/片区名称】为空的OSD，请重新选择");
						return ;
					 }
		...
		function deleteOsd(ids){
			var url = "<%=path%>/boss/message/ajax/message_ajax_delete_osd.jsp?ids=" + ids;
			send_request1(url);
		}
		
		function sendOSDs(ids){
			var url = "<%=path%>/boss/message/ajax/message_ajax_send_osd.jsp?ids=" + ids;
			send_request1(url);
		}
			...
			{ // 判断对象状态
				...
				{ // 信息已经成功返回，开始处理信息
					...
					if(result == "fail"){
						...
						alert("删除成功");
						window.location.href = "<%= path %>/bossCoreMessage/osd.do?method=list";
					}
				} else 
				{ //页面不正常
					alert("您所请求的页面有异常。");
