...
<script type="text/javascript">
	 ...
	 */	
	 function 	saveFeeType() 
	 {
		 ...
		 var realIndexId = '<c:out value="${dayFeeType.indexId}"/>' ;  
		 var daySeqNo = "";
		 var dayStandard = "";
		 ...
		 for ( var i = 0 ; i <divArray.length ; i ++ )
		 {
			 ...
			 if ( realIndexId == '' ){
			  ...
			  if (div0Inputs[0+modify].value =='' || div0Inputs[1+modify].value==''|| div0Inputs[2+modify].value==''){
				...
				{
					saveData+=div0Inputs[0+modify].value+"@"+div0Inputs[1+modify].value+"@"+div0Inputs[2+modify].value+"@"+div0Selects[0].value;
				}
				daySeqNo = div0Inputs[div0Inputs.length -1].value;
				dayStandard = div0Selects[1].value;
				if(daySeqNo != "" && !checkNumFlag(daySeqNo)){
					return;
				}
				if(div0Selects[0].value == '0')
				{
					saveData+= ("@" + daySeqNo + "@" + dayStandard);
				    ...
				    **/		
					...
					for(var n =0 ; n < tempSelect.length/2 ; n=n+1){
						...
						//alert(realFee[2]);
						if(realSign == '1'){
							realFinal = parseFloat(realFee[2]) - parseFloat(realVal);
							}
						if(realSign == '2'){
							realFinal = parseFloat(realFee[2]) * parseFloat(realVal);
							}
			     ...
			     <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			    		  ...
			    		  <div id="month"  style="display:block" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  ...
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">按天收费策略信息定义
								  		<c:if test="${ dayFeeType.OPER !=   'modify' }">
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_commodity_name_month1" id="month_commodity_name_month1" value="<c:out value="${dayFeeType.commodity_name }" />" size="30" maxlength="50"/>
						...
						<td width="37%" colspan="1">
						  <input type="text" name="month_chargeCycle_month1" value="1"  size="30" maxlength="9" readonly  />(天)
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_price_month1" value="<c:out value="${dayFeeType.price }" />" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
						...
						<td width="37%" colspan="1">
							<select name="month_rebate_type_month1"  id = "month_rebate_type_month1" onchange="showDiscountPolicy( this.options[this.selectedIndex].value, 'month' , 'month1')" >
								<option  <c:if test="${ dayFeeType.rebate_type eq 0  }"> selected="selected" </c:if> value="0">不优惠</option>
								<option  <c:if test="${ dayFeeType.rebate_type eq 2  }"> selected="selected" </c:if>  value="2">服务优惠</option>
							</select> 
						</td>
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_daySeqNo" 
							<c:if test="${ dayFeeType.daySeqNo != 0  }">value="<c:out value="${ dayFeeType.daySeqNo}"/>"</c:if>
							<c:if test="${ dayFeeType.daySeqNo == 0 }">value="<c:out value=""/>"</c:if> size="30" maxlength="9"  onKeyUp="clearNoNum(event,this)"/>
						           请输入1~99999之间的整数
						</td>
						<td width="12%" align="right">
							是否为标准策略
						...
						<td width="37%" colspan="1">
							<select name="month_service_dayStandard"  id = "month_service_dayStandard">
								<option  <c:if test="${ dayFeeType.dayStandard eq 0  }"> selected="selected" </c:if> value="0">否</option>
								<option  <c:if test="${ dayFeeType.dayStandard eq 1  }"> selected="selected" </c:if> value="1">是</option>
							</select> 
						</td>
					</tr>
			    		...
			    		<td>
			    		  <div id="month_discount_policy_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">按天收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
...
<%--																		   		 onclick = "showDiscountDiv(  document.getElementsByName( 'month_discountPolicyDefineId_month1' ) , 'month' , 'false' , 'month1')" value="by_user_type"   />按用户类型优惠--%>
			    		...
			    		<td>
			    		  <div id="month_by_user_type_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">用户类型按天收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
				              							...
				              							<c:when test="${fn:length(sessionScope.userTypeParaList)   > 0  }">
				              								<tr >
																	...
																	<td width="37%" colspan="1">
																		<select name="month_policy_pattern_month1" >
																			<option <c:if test="${ dayFeeType.userTypePattern eq 2  }"> selected="selected" </c:if> value="2">封顶</option>
																			<option <c:if test="${ dayFeeType.userTypePattern eq 1  }"> selected="selected" </c:if>  value="1">打折</option>
																		</select>
				              								...
				              								<c:forEach var="userPara" items="${sessionScope.userTypeParaList}" varStatus="index">
																<tr>
																	...
																	<td width="37%" colspan="1">
																		<input type="text" name="month_by_user_type<c:out value="${ index }"/>_month1" value="<c:out value="${ dayFeeType.userTypeCodeValueMap[userPara.value] }"></c:out>" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>
																			1~300个字符(数字、小数点),数字开头<font color="#FF0000">*</font>
																	</td>
																</tr>
															</c:forEach>
			    		...
			    		<td>
			    		  <div id="month_by_equ_type_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">终端类型按天收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
				              							...
				              							<c:when test="${ fn:length(sessionScope.equTypeParaList)   > 0  }">
				              								<tr >
																	...
																	<td width="37%" colspan="1">
																		<select name="month_equ_type_policy_pattern_month1" >
																			<option <c:if test="${ dayFeeType.equTypePattern eq 2  }"> selected="selected" </c:if> value="2">封顶</option>
																			<option <c:if test="${ dayFeeType.equTypePattern eq 1  }"> selected="selected" </c:if> value="1">打折</option>
																		</select>
				              								...
				              								<c:forEach var="equPara" items="${ sessionScope.equTypeParaList }" varStatus="index">
																<tr>
																	...
																	<td width="37%" colspan="1">
																		<input type="text" name="month_by_equ_type<c:out value="${ index }"/>_month1" value="<c:out value="${ dayFeeType.equTypeCodeValueMap[  equPara.value ] }"></c:out>" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>
																		1~300个字符(数字、小数点),数字开头<font color="#FF0000">*</font>
																	</td>
																</tr>
															</c:forEach>
			    		...
			    		<td>
			    		  <div id="month_by_data_rights_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">操作员按天收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
		 ...
		 <table width="30%" border="0" cellspacing="0" cellpadding="0">
				 <tr align='center'> 
				 	<c:if test="${ dayFeeType.OPER !=   'modify' }">  <app:button title="增加" image="" function="addFeeType()"/> </c:if>
