		 ...
		 for ( var i = 0 ; i <divArray.length ; i ++ )
		 {
			 ...
			 if ( realIndexId == '' ){
			  ...
			  if (div0Inputs[0+modify].value =='' || div0Inputs[1+modify].value==''){
				...
				{
					saveData+=div0Inputs[0+modify].value+"@"+div0Selects[0].value+"@"+div0Inputs[1+modify].value+"@"+div0Selects[1].value+"@"+div0Selects[2].value+"@"+div0Selects[3].value;
				}
				rebateSeqNo = div0Inputs[div0Inputs.length-1].value;
				if(rebateSeqNo != "" && !checkNumFlag(rebateSeqNo)){
					return;
				}
				if(div0Selects[3].value == '0' || div0Selects[3].value == '2' )
				{
					saveData+= "@" + rebateSeqNo;
			     ...
			     <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			    	<tr> 
			    		  ...
			    		  <div id="month"  style="display:block" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  ...
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">套餐打折收费策略信息定义
						...
						<td width="37%" colspan="1">
							<input type="text" name="package_rebate_commodity_name_month1" id="package_rebate_commodity_name_month1" value="<c:out value="${rebateFeeType.commodity_name }" />" size="30" maxlength="50"/>
						...
						<td width="37%" colspan="1">
							<select name="package_rebate_reorder_type_month1" >
								<option  <c:if test="${ rebateFeeType.reOrder_type eq 1  }"> selected="selected" </c:if> value="1">本策略续订</option>
								<option <c:if test="${  rebateFeeType.reOrder_type eq 0 }">  selected="selected" </c:if>  value="0">不续订</option>
							</select>
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="package_rebate_charge_value_month1" value="<c:out value="${rebateFeeType.charge_value }" />" size="30" maxlength="50" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)" />折(精确二位小数)
						...
						<td width="37%" colspan="1">
							<select name="package_rebate_activate_type_month1" >
								<option <c:if test="${ rebateFeeType.activate_type eq 0  }"> selected="selected" </c:if> value="0">即时生效</option>
								<option <c:if test="${ rebateFeeType.activate_type eq 1  }"> selected="selected" </c:if> value="1">下月生效</option>
							</select>
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<select name="month_stop_postphone_month1" >
								...
								<option  <c:if test="${ rebateFeeType.stop_postphone eq 0  }"> selected="selected" </c:if> value="0">退费顺延</option>
								<option <c:if test="${rebateFeeType.stop_postphone  eq 1  }"> selected="selected" </c:if>  value="1">退费不顺延</option>
								<option <c:if test="${ rebateFeeType.stop_postphone  eq 2  }"> selected="selected" </c:if>  value="2">不退费</option>	
						...
						<td width="37%" colspan="1">
							<select name="month_rebate_type_month1"  id = "month_rebate_type_month1" onchange="showDiscountPolicy( this.options[this.selectedIndex].value, 'month' , 'month1')" >
								<option  <c:if test="${ rebateFeeType.rebate_type eq 0  }"> selected="selected" </c:if> value="0">不优惠</option>
								<option  <c:if test="${ rebateFeeType.rebate_type eq 1  }"> selected="selected" </c:if>  value="1">套餐优惠</option>
								<option  <c:if test="${ rebateFeeType.rebate_type eq 2  }"> selected="selected" </c:if>  value="2">服务优惠</option>
								<option  <c:if test="${ rebateFeeType.rebate_type eq 3  }"> selected="selected" </c:if>  value="3">双重优惠</option>
							</select> 
						</td>
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_rebateSeqNo" 
							<c:if test="${ rebateFeeType.rebateSeqNo != 0 }">value="<c:out value="${ rebateFeeType.rebateSeqNo}"/>"</c:if>
							<c:if test="${ rebateFeeType.rebateSeqNo == 0 }">value="<c:out value=""/>"</c:if> size="30" maxlength="9"  onKeyUp="clearNoNum(event,this)"/>
						            请输入1~99999之间的整数
						</td>
					</tr>
				                					</table>
			    		...
			    		<td>
			    		  <div id="month_discount_policy_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">套餐打折收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
			    		...
			    		<td>
			    		  <div id="month_by_user_type_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">用户类型套餐打折收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
			    		...
			    		<td>
			    		  <div id="month_by_equ_type_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">终端类型套餐打折收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
			    		...
			    		<td>
			    		  <div id="month_by_data_rights_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">操作员套餐打折收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
