...
<script type="text/javascript">
	...
	function showTransferPolicy( reorderType , currentIndexId  , selectedIndexId , divPreFix ) 
	{
		if ( reorderType != 2 ){
			document.getElementById( 'transferPolicyName'+divPreFix ).innerHTML = '';
			// document.getElementById(   'transferPolicyName'  ).style.display = 'none' ;  
			return ;
		}	
		document.getElementById(   'transferPolicyName'+divPreFix   ).style.display = 'inline' ;  
		document.getElementById( 'transferPolicyName'+divPreFix  ).innerHTML = '转策略名称' +  window.dialogArguments.getTransferPolicyHtml( currentIndexId, selectedIndexId  ) ;
	}
	 ...
	 */	
	 function 	saveFeeType() 
	 {
		 ...
		 var realIndexId = '<c:out value="${multiMonthFeeType.indexId}"/>' ;  
		 ...
		 for ( var i = 0 ; i <divArray.length ; i ++ )
		 {
		   if(  divArray[i].id != 'month' &&  !divArray[i].id.startWith('month_') && divArray[i].id.startWith('month') ) {
			 ...
			 if ( realIndexId == '' ){
			  ...
			  var div0Inputs = inDivArray[0].getElementsByTagName("input");
	 			...
	 			var div0RebateTypeValue = '' ;
	 			var multiSeqNo = "";
	 			var multiStandard = "";
	 			var yearStandardValue = "";
	 			var monthStandardValue = "";
	 			var dayStandardValue = "";
			  ...
			  if (div0Inputs[0+modify].value =='' || div0Inputs[1+modify].value==''|| div0Inputs[2+modify].value==''){
				...
				{
					if ( div0Selects[0].value == 2 &&  div0Selects[1].value == '' )
					{
						alert( '必须选择一个转策略！');
						return ;
					}  

					if ( div0Selects[0].value == 2 )
					{
						div0RebateTypeValue = div0Selects[3].value; 
						saveData+=div0Inputs[0+modify].value+"@"+div0Selects[0].value+"@"+div0Selects[1].value
				  		+"@"+div0Inputs[1+modify].value+"@"+div0Inputs[2+modify].value
				  		+"@"+div0Selects[2].value+"@"+div0Inputs[3+modify].value+"@"+div0Inputs[4+modify].value
				  		+"@"+div0Selects[3].value;
				  		multiStandard = div0Selects[4].value;
					}else{
						div0RebateTypeValue = div0Selects[2].value; 
						
						saveData+=div0Inputs[0+modify].value+"@"+div0Selects[0].value 
				  		+"@"+div0Inputs[1+modify].value+"@"+div0Inputs[2+modify].value
				  		+"@"+div0Selects[1].value+"@"+div0Inputs[3+modify].value+"@"+div0Inputs[4+modify].value
				  		+"@"+div0Selects[2].value;
				  		multiStandard = div0Selects[3].value;
					}
					multiSeqNo = div0Inputs[div0Inputs.length-4].value;
					yearStandardValue = div0Inputs[div0Inputs.length-3].value;
					monthStandardValue = div0Inputs[div0Inputs.length-2].value;
					dayStandardValue = div0Inputs[div0Inputs.length-1].value;
					if(multiSeqNo != "" && !checkNumFlag(multiSeqNo)){
						return;
					}
				}
				if(div0RebateTypeValue  == '0')
				{	
					saveData+=("@"+multiSeqNo + "@" + multiStandard+"@"+yearStandardValue+"@"+monthStandardValue+"@"+dayStandardValue);
				    ...
				    **/		
					...
					for(var n =0 ; n < tempSelect.length/2 ; n=n+1){
						...
						if(realFee.length<13){
							
							if(realSign == '1'){
								realFinal = parseFloat(realFee[3]) - parseFloat(realVal);
								} 
							if(realSign == '2'){
								realFinal = parseFloat(realFee[3]) * parseFloat(realVal);
								}
							...
							if(realSign == '1'){
								realFinal = parseFloat(realFee[4]) - parseFloat(realVal);
								} 
							if(realSign == '2'){
								realFinal = parseFloat(realFee[4]) * parseFloat(realVal);
								}
					...
					saveData+= "@"+ "ccc";
					saveData+= "@" + multiSeqNo;
					saveData+= "@" + multiStandard;
					saveData+= "@" + yearStandardValue;
					saveData+= "@" + monthStandardValue;
					saveData+= "@" + dayStandardValue;
					/**
		  ...
		  var valueArray =  getTransferPolicyIdNameMap();


		  <%
		 	request.getSession().removeAttribute("indexYhclId") ;
		  %>
		  window.dialogArguments.addFeeType(  feeTypeData  ,realIndexId , valueArray ,   <%=request.getParameter("feeTypeId") %> );	
		  window.close();
	 }

	    function getTransferPolicyIdNameMap ( ) 
	    {
		    var valueArray = new Array();
	    	var obj=document.getElementById('transferPolicy');
	    	for(i=0;  obj != null  && i<obj.length;i++){
	    		 valueArray[ obj[i].value ] = obj[i].innerText;
	    	}
			     ...
			     <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			    		  ...
			    		  <div id="month"  style="display:block" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  ...
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">一次性收费策略信息定义
								  	<c:if test="${ multiMonthFeeType.OPER !=   'modify' }"><input type = 'button' onclick="delFeeType(  'month1'    )"  value='删除' /></c:if>  </span></td>
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_commodity_name_month1" id="month_commodity_name_month1" value="<c:out value="${multiMonthFeeType.commodity_name }" />" size="30" maxlength="50"/>
						...
						<td width="44%" colspan="1" >
							<select name="month_reorder_type_month1" onchange="showTransferPolicy( this.options[this.selectedIndex].value , '<c:out value="${multiMonthFeeType.indexId}"/>' , '' , '_month1')"  >
								<option <c:if test="${ multiMonthFeeType.reorder_type eq 0  }"> selected="selected" </c:if> value="0">不续订</option>
								<option <c:if test="${ multiMonthFeeType.reorder_type eq 2   }"> selected="selected" </c:if> value="2">转策略续订</option>
								<option <c:if test="${ multiMonthFeeType.reorder_type eq 1  }"> selected="selected" </c:if> value="1">本策略续订</option>
							</select>					   
						   <span   	id="transferPolicyName_month1" style="display:none">
						   </span>
						</td>
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_chargeCycle_month1" value="<c:out value="${multiMonthFeeType.serviceTerm }" />" size="30" maxlength="9"  onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(个月)
						    1~9个数字字符 <font color="#FF0000">*</font>
						</td>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_price_month1" value="<c:out value="${multiMonthFeeType.servicePrice }" />" size="30" maxlength="300"  onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<select name="month_stop_postphone_month1" >
						        <option <c:if test="${ multiMonthFeeType.stop_postphone  eq 2  }"> selected="selected" </c:if>  value="2">不退费</option>
								<option  <c:if test="${ multiMonthFeeType.stop_postphone eq 0  }"> selected="selected" </c:if> value="0">退费顺延</option>
								<option <c:if test="${ multiMonthFeeType.stop_postphone  eq 1  }"> selected="selected" </c:if>  value="1">退费不顺延</option>
							</select>
						...
						<td width="37%" colspan="1">
							<input type="hidden" id="relativeServiceIds" name="relativeServiceIds" value="<c:out value="${multiMonthFeeType.relFeeTypeId }" />"/>
							<input type="text"  id="relativeServiceIds_helpLabel" name="relativeServiceIds_helpLabel"  value="<c:out value="${multiMonthFeeType.relFeeTypeName }" />"  onkeypress=" event.returnValue = false "   size="30" maxlength="50" /> 
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<select name="month_rebate_type_month1"  onchange="showDiscountPolicy( this.options[this.selectedIndex].value, 'month' , 'month1')" >
								<option  <c:if test="${ multiMonthFeeType.rebate_type eq 0  }"> selected="selected" </c:if> value="0">不优惠</option>
								<option  <c:if test="${ multiMonthFeeType.rebate_type eq 2  }"> selected="selected" </c:if>  value="2">服务优惠</option>
							</select> 
						</td>
						<td width="12%" align="right">
							序号
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_multSeqNo" 
							<c:if test="${ multiMonthFeeType.multiSeqNo != 0  }">value="<c:out value="${ multiMonthFeeType.multiSeqNo}"/>"</c:if>
							<c:if test="${ multiMonthFeeType.multiSeqNo == 0 }">value="<c:out value=""/>"</c:if> size="30" maxlength="9"  onKeyUp="clearNoNum(event,this)"/>
						           请输入1~99999之间的整数
						</td>
					</tr>
					<tr>
						...
						<td width="37%" colspan="1">
							<select name="month_service_multiStandard"  id = "month_service_multiStandard">
								<option  <c:if test="${ multiMonthFeeType.multiStandard eq 0  }"> selected="selected" </c:if> value="0">否</option>
								<option  <c:if test="${ multiMonthFeeType.multiStandard eq 1  }"> selected="selected" </c:if>  value="1">是</option>
							</select> 
						</td>
						<td width="12%" align="right">
							年标准服务价格
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_multiYearStandardValue" value="<c:out value="${multiMonthFeeType.multiYearStandardValue }" />" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
						               (数字、小数点),数字开头
						</td>
					</tr>
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_multiMonthStandardValue" value="<c:out value="${multiMonthFeeType.multiMonthStandardValue }" />" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
						               (数字、小数点),数字开头
						</td>
						<td width="12%" align="right">
							天标准服务价格
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_multiDayStandardValue" value="<c:out value="${multiMonthFeeType.multiDayStandardValue }" />" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
						               (数字、小数点),数字开头
						</td>
					</tr>
			    		...
			    		<td>
			    		  <div id="month_discount_policy_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">一次性收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
...
<%--																		   		 onclick = "showDiscountDiv(  document.getElementsByName( 'month_discountPolicyDefineId_month1' ) , 'month' , 'false' , 'month1')" value="by_user_type"   />按用户类型优惠--%>
			    		...
			    		<td>
			    		  <div id="month_by_user_type_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">用户类型一次性收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
				              							...
				              							<c:when test="${fn:length(sessionScope.userTypeParaList)   > 0  }">
				              								<tr >
																	...
																	<td width="37%" colspan="1">
																		<select name="month_policy_pattern_month1" >
																			<option <c:if test="${ multiMonthFeeType.userTypePattern eq 2  }"> selected="selected" </c:if> value="2">封顶</option>
																			<option <c:if test="${ multiMonthFeeType.userTypePattern eq 1  }"> selected="selected" </c:if>  value="1">打折</option>
																		</select>
				              								...
				              								<c:forEach var="userPara" items="${sessionScope.userTypeParaList}" varStatus="index">
																<tr>
																	...
																	<td width="37%" colspan="1">
																		<input type="text" name="month_by_user_type<c:out value="${ index }"/>_month1" value="<c:out value="${ multiMonthFeeType.userTypeCodeValueMap[userPara.value] }"></c:out>" size="30" maxlength="300"  onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>
																		 1~300个字符(数字、小数点),数字开头<font color="#FF0000">*</font>
																	</td>
																</tr>
															</c:forEach>
			    		...
			    		<td>
			    		  <div id="month_by_equ_type_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">终端类型一次性收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
				              							...
				              							<c:when test="${ fn:length(sessionScope.equTypeParaList)   > 0  }">
				              								<tr >
																	...
																	<td width="37%" colspan="1">
																		<select name="month_equ_type_policy_pattern_month1" >
																			<option <c:if test="${ multiMonthFeeType.equTypePattern eq 2  }"> selected="selected" </c:if> value="2">封顶</option>
																			<option <c:if test="${ multiMonthFeeType.equTypePattern eq 1  }"> selected="selected" </c:if> value="1">打折</option>
																		</select>
				              								...
				              								<c:forEach var="equPara" items="${ sessionScope.equTypeParaList }" varStatus="index">
																<tr>
																	...
																	<td width="37%" colspan="1">
																		<input type="text" name="month_by_equ_type<c:out value="${ index }"/>_month1" value="<c:out value="${ multiMonthFeeType.equTypeCodeValueMap[  equPara.value ] }"></c:out>" size="30" maxlength="300"  onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>
																		 1~300个字符(数字、小数点),数字开头<font color="#FF0000">*</font>
																	</td>
																</tr>
															</c:forEach>
			    		...
			    		<td>
			    		  <div id="month_by_data_rights_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">操作员一次性收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
		 ...
		 <table width="30%" border="0" cellspacing="0" cellpadding="0">
				 <tr align='center'> 
				 	 <c:if test="${ multiMonthFeeType.OPER !=   'modify' }"> <app:button title="增加" image="" function="addFeeType()"/></c:if>
      ...
      <script type="text/javascript">
		...
		var realIndexId = '<c:out value="${multiMonthFeeType.indexId}"/>' ;  
		var selectIndexId =  '<c:out value="${multiMonthFeeType.transfer_policy }"/>' ; 
		var reorderType = '<c:out value="${multiMonthFeeType.reorder_type }"/>' ; 
		if ( selectIndexId != '' && realIndexId != '' )
		{
			showTransferPolicy( reorderType ,realIndexId ,  selectIndexId , '_month1') ;
		} 
