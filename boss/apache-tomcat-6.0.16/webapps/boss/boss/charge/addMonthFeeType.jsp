...
<script type="text/javascript">
	 ...
	 */	
	 function 	saveFeeType() 
	 {
		 ...
		 var realIndexId = '<c:out value="${monthFeeType.indexId}"/>' ;  
		 var monthSeqNo = "";
		 var monthStandard = "";
		 ...
		 for ( var i = 0 ; i <divArray.length ; i ++ )
		 {
			 ...
			 if ( realIndexId == '' ){
			  ...
			  if (div0Inputs[0+modify].value =='' || div0Inputs[1+modify].value==''|| div0Inputs[2+modify].value==''){
				...
				{
					if ( div0Inputs[4+modify].value == '' )
						div0Inputs[3+modify].value= '' ;
					
					saveData+=div0Inputs[0+modify].value+"@"+div0Selects[0].value+"@"+div0Inputs[1+modify].value+"@"+div0Inputs[2+modify].value+"@"+div0Selects[1].value
							+"@"+div0Inputs[3+modify].value+"@"+div0Inputs[4+modify].value+"@"+div0Selects[2].value;
				}
				monthSeqNo = div0Inputs[div0Inputs.length-3].value;
				monthStandard = div0Selects[3].value;
				monthStandardValue = div0Inputs[div0Inputs.length-2].value;
				dayStandardValue = div0Inputs[div0Inputs.length-1].value;
				
				if(monthSeqNo != "" && !checkNumFlag(monthSeqNo)){
					return;
				}
				if(div0Selects[2].value == '0')
				{
					saveData += ("@" + monthSeqNo + "@" + monthStandard+"@"+monthStandardValue+"@"+dayStandardValue);
			     ...
			     <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			    		  ...
			    		  <div id="month"  style="display:block" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  ...
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">按月收费策略信息定义
								  		<c:if test="${ monthFeeType.OPER !=   'modify' }">
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_commodity_name_month1" id="month_commodity_name_month1" value="<c:out value="${monthFeeType.commodity_name }" />" size="30" maxlength="50"/>
						...
						<td width="37%" colspan="1">
							<select name="month_reorder_type_month1" >
								<option  <c:if test="${ monthFeeType.reorder_type eq 1  }"> selected="selected" </c:if> value="1">本策略续订</option>
								<option <c:if test="${ monthFeeType.reorder_type  eq 0  }"> selected="selected" </c:if>  value="0">不续订</option>
							</select>
					...
					<tr>
						...
						<td width="37%" colspan="1">
						<!-- 
						 <input type="text" name="month_chargeCycle_month1" value="<c:out value="${monthFeeType.chargeCycle }" />"  size="30" maxlength="9" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)" />(个月)
						  	1~9个数字字符 <font color="#FF0000">*</font>
						 -->
						  <input type="text" name="month_chargeCycle_month1" value="1"  size="30" maxlength="9" readonly  />(个月)
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_price_month1" value="<c:out value="${monthFeeType.price }" />" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
					   ...
					   <td width="12%" align="right">
						...
						<td width="37%" colspan="1">
							<select name="month_stop_postphone_month1" >
								<option <c:if test="${ monthFeeType.stop_postphone  eq 2  }"> selected="selected" </c:if>  value="2">不退费</option>
								<option  <c:if test="${ monthFeeType.stop_postphone eq 0  }"> selected="selected" </c:if> value="0">退费顺延</option>
								<option <c:if test="${ monthFeeType.stop_postphone  eq 1  }"> selected="selected" </c:if>  value="1">退费不顺延</option>
							</select>
						...
						<td width="37%" colspan="1">
							<input type="hidden" id="relativeServiceIds" name="relativeServiceIds" value="<c:out value="${monthFeeType.relFeeTypeId }" />"/>
							<input type="text"  id="relativeServiceIds_helpLabel" name="relativeServiceIds_helpLabel"  value="<c:out value="${monthFeeType.relFeeTypeName }" />"  onkeypress=" event.returnValue = false "   size="30" maxlength="50" /> 
							<input type="button" name="popSelect" id="popSelect" value="..." 
										onclick="window.open( '<%=request.getContextPath()%>/jsp/boss/charge/productRelServ.do?methodToCall=openQuery&editTypeId=relativeServiceIds&serviceId=<%=request.getParameter("serviceId") %>&feeTypeId=<%=request.getParameter("feeTypeId") %>' ,  
					...
					<tr>
						...
						<td width="37%" colspan="1">
							<select name="month_rebate_type_month1"  id = "month_rebate_type_month1" onchange="showDiscountPolicy( this.options[this.selectedIndex].value, 'month' , 'month1')" >
								<option  <c:if test="${ monthFeeType.rebate_type eq 0  }"> selected="selected" </c:if> value="0">不优惠</option>
								<option  <c:if test="${ monthFeeType.rebate_type eq 2  }"> selected="selected" </c:if>  value="2">服务优惠</option>
							</select> 
						</td>
						<td width="12%" align="right">
							序号
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_service_monthSeqNo" 
							<c:if test="${ monthFeeType.monthSeqNo != 0  }">value="<c:out value="${ monthFeeType.monthSeqNo}"/>"</c:if>
							<c:if test="${ monthFeeType.monthSeqNo == 0 }">value="<c:out value=""/>"</c:if> size="30" maxlength="9"  onKeyUp="clearNoNum(event,this)"/>
						          请输入1~99999之间的整数
						</td>
					</tr>
					<tr>
						...
						<td width="37%" colspan="1">
							<select name="month_service_monthStandard"  id = "month_service_monthStandard">
								<option  <c:if test="${ monthFeeType.monthStandard eq 0  }"> selected="selected" </c:if> value="0">否</option>
								<option  <c:if test="${ monthFeeType.monthStandard eq 1  }"> selected="selected" </c:if>  value="1">是</option>
							</select> 
						</td>
						<td width="12%" align="right">
							月标准计费价格
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_price_monthStandardValue" value="<c:if test="${ monthFeeType.monthStandardValue eq 0.0  }"><c:out  value=""/></c:if><c:if test="${ monthFeeType.monthStandardValue ne 0.0  }"><c:out value="${monthFeeType.monthStandardValue }"/></c:if>" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
						               (数字、小数点),数字开头
						</td>
					</tr>
					<tr>
						...
						<td width="37%" colspan="1">
							<input type="text" name="month_price_dayStandardValue" value="<c:if test="${ monthFeeType.dayStandardValue eq 0.0  }"><c:out  value=""/></c:if><c:if test="${ monthFeeType.dayStandardValue ne 0.0  }"><c:out value="${monthFeeType.dayStandardValue }"/></c:if>" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>(元)
						               (数字、小数点),数字开头
						</td>
					</tr>
...
<%--																		   		 onclick = "showDiscountDiv(  document.getElementsByName( 'month_discountPolicyDefineId_month1' ) , 'month' , 'false' , 'month1')" value="by_user_type"   />按用户类型优惠--%>
				              							...
				              							<c:when test="${ fn:length(sessionScope.equTypeParaList)   > 0  }">
				              								<tr >
																	...
																	<td width="37%" colspan="1">
																		<select name="month_equ_type_policy_pattern_month1" >
																			<option <c:if test="${ monthFeeType.equTypePattern eq 2  }"> selected="selected" </c:if> value="2">封顶</option>
																			<option <c:if test="${ monthFeeType.equTypePattern eq 1  }"> selected="selected" </c:if> value="1">打折</option>
																		</select>
				              								...
				              								<c:forEach var="equPara" items="${ sessionScope.equTypeParaList }" varStatus="index">
																<tr>
																	...
																	<td width="37%" colspan="1">
																		<input type="text" name="month_by_equ_type<c:out value="${ index }"/>_month1" value="<c:out value="${ monthFeeType.equTypeCodeValueMap[  equPara.value ] }"></c:out>" size="30" maxlength="300" onKeyUp="clearNoNum(event,this)" onBlur="checkNum(this)"/>
																		1~300个字符(数字、小数点),数字开头<font color="#FF0000">*</font>
																	</td>
																</tr>
															</c:forEach>
			    		...
			    		<td>
			    		  <div id="month_by_data_rights_month1"  style="display:none" > 
			       			<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
			         			<tr> 
								  <td width="6" height="25"><img src="<%= path%>/common/images/bar_left.gif" width="31" height="23"></td>
								  <td width="100%" background="<%= path%>/common/images/bar_bg.gif"><span class="L style2">操作员按月收费优惠策略信息定义</span></td>
								  <td width="6" align="right"><img src="<%= path%>/common/images/bar_right.gif" width="9" height="23"></td>
						   		</tr>
			         			<tr>
			           				<td colspan="3">
		 ...
		 <table width="30%" border="0" cellspacing="0" cellpadding="0">
				 <tr align='center'> 
				 	<c:if test="${ monthFeeType.OPER !=   'modify' }">  <app:button title="增加" image="" function="addFeeType()"/> </c:if>
   ...
   <script type="text/javascript">
		...
		function deleteItem(ids){
			if($(ids).parent().parent().prev().prev()[0]!=undefined){
				$(ids).parent().parent().remove();
			}
			else{
				alert("最后一个优惠策略不能删除，如需删除，请选择无优惠！")
			}
		}
